...
# This tool supports operator-configurable FREQUENCY and MODE settings for
# SIM/eSIM/USIM issuance. These parameters are essential for compliance
# with international and carrier-specific requirements (e.g., 2G/3G/4G/5G,
# region bands, test/production modes).
...
# MODE: Specifies the operational mode (e.g., 'GSM', 'UMTS', 'LTE', '5G',
# 'TEST', 'PRODUCTION').
...
class SimCardIssuer:
    ...
    # Use via CLI for normal operation, or via Python API for
    ...
    # Keyset file references for configuration (used for carrier/cert
    # personalization)
    KEYSET_FILES = {
        # File with hex string for Ki (subscriber authentication key)
        "Ki": "tmo_ki.hex",
        # File with hex string for OPc (operator variant key)
        "OPc": "tmo_opc.hex",
    ...
    def sim_apdu_mode(self, emulator=False):
        ...
        # Operator is guided through file selection and update for IMSI, Kc,
        # SPN, etc.
        self.log(
            "--- Classic SIM Personalization (3GPP TS 51.011/ETSI TS 102 221) ---")
        sim_apdus = [
            # Select MF (Master File)
            ([0x00, 0xA4, 0x00, 0x00, 0x02, 0x3F, 0x00], "Select MF (Master File)"),
            # Select DF_GSM (GSM Directory)
            ([0x00, 0xA4, 0x00, 0x00, 0x02, 0x7F, 0x20],
             "Select DF_GSM (GSM Directory)"),
            ...
            ([0x00, 0xD6, 0x00, 0x00, 0x09] + [0x21, 0x43, 0x65, 0x87,
             0x09, 0x21, 0x43, 0x65, 0xF9], "Update Binary (IMSI example)"),
            # Select EF_Kc (Ciphering Key)
            ([0x00, 0xA4, 0x02, 0x00, 0x02, 0x6F, 0x20],
             "Select EF_Kc (Ciphering Key)"),
            # Update Binary (Kc, example value)
            ([0x00, 0xD6, 0x00, 0x00, 0x09] + [0xAA]
             * 9, "Update Binary (Kc example)"),
            # Select EF_SPN (Service Provider Name)
            ([0x00, 0xA4, 0x02, 0x00, 0x02, 0x6F, 0x46],
             "Select EF_SPN (Service Provider Name)"),
    ...
    def umts_apdu_mode(self, emulator=False):
        ...
        # Implements 3GPP TS 31.102. Operator is guided through file selection
        # and update for IMSI, AD, authentication, etc.
        self.log("--- UMTS/USIM Personalization (3GPP TS 31.102/Legacy 3G) ---")
        umts_apdus = [
            # Select MF (Master File)
            ([0x00, 0xA4, 0x00, 0x00, 0x02, 0x3F, 0x00], "Select MF (Master File)"),
            # Select DF_USIM (USIM Directory)
            ([0x00, 0xA4, 0x00, 0x00, 0x02, 0x7F, 0xFF],
             "Select DF_USIM (USIM Directory)"),
            # Select EF_IMSI (IMSI file)
            ...
            ([0x00, 0xA4, 0x02, 0x00, 0x02, 0x6F, 0xAD],
             "Select EF_AD (Administrative Data)"),
            # Update Binary (AD, example value)
            ([0x00, 0xD6, 0x00, 0x00, 0x04] +
             [0x00, 0x00, 0x00, 0x00], "Update Binary (AD example)"),
            # Select EF_USIM_AUTH (Authentication)
            ([0x00, 0xA4, 0x02, 0x00, 0x02, 0x6F, 0x20],
             "Select EF_USIM_AUTH (Authentication)"),
            # RUN UMTS ALGORITHM (3G Auth, RAND/AUTN)
            ([0x00, 0x88, 0x00, 0x81, 0x20] + [0x02] * 16 + [0x03] * 16,
             "RUN UMTS ALGORITHM (3G Auth, RAND=0x02.., AUTN=0x03..)")
        ]
    ...
    def generate_imsi(mcc="310", mnc="260"):
        ...
        msin = ''.join(str(random.randint(0, 9))
                       for _ in range(15 - len(mcc + mnc)))
        return mcc + mnc + msin
    ...
    def __init__(self):
        ...
        if EmergencyFunctionManager:
            try:
                ...
                self.log(
                    f"[Warning] Emergency manager initialization failed: {e}")

    def load_config(self):
        # Load the configuration from disk (JSON file). Used for
    ...
    def send_apdu(self, apdu, description=None, emulator=False):
        ...
        if emulator:
            ...
            self.log(
                f"[EMULATOR] Simulated Response: {resp_str}, SW: {
                    sw1:02X} {
                    sw2:02X}")
            return data, sw1, sw2
    ...
    def issuer_mode(self, emulator=False, export_android=None):
        ...
        # Note: Real cryptographic values must be generated securely in
        # production.
        ...
        apdu_sequence = [
            ...
            ([0x00, 0xD6, 0x00, 0x00, 0x02, 0x12, 0x34],
             "Update Binary (write 2 bytes to MF at offset 0)"),
            # Verify CHV1 (PIN1, default 8x'FF')
            ([0x00, 0x20, 0x00, 0x01, 0x08, 0xFF, 0xFF, 0xFF, 0xFF,
             0xFF, 0xFF, 0xFF, 0xFF], "Verify CHV1 (PIN1, default)"),
            # Change CHV1 (PIN1)
            ([0x00, 0x24, 0x00, 0x01, 0x10] + [0xFF] * 8 +
             [0x12] * 8, "Change CHV1 (PIN1, old=FF, new=12)"),
            # Disable CHV1
            ([0x00, 0x26, 0x00, 0x01, 0x08] +
             [0x12] * 8, "Disable CHV1 (PIN1=12)"),
            # Enable CHV1
            ...
            ([0x00, 0xB0, 0x00, 0x00, 0x0A],
             "Read Binary (first 10 bytes of DF_GSM)"),
            # Update Binary (write 2 bytes to DF_GSM at offset 0)
            ([0x00, 0xD6, 0x00, 0x00, 0x02, 0x56, 0x78],
             "Update Binary (write 2 bytes to DF_GSM at offset 0)"),
            ...
            ([0x00, 0x88, 0x00, 0x00, 0x10] + [0x01] * 16,
             "RUN GSM ALGORITHM (2G Auth, RAND=0x01..0x10)"),
        ...
        # Iterate through all MF, DF_GSM, and encryption APDUs, operator sees
        # each action
        ...
        # Each step is commented and can be adapted for your SM-DP/SM-DS
        # implementation
        ...
        self.send_apdu(
            select_isdp,
            "GSMA SGP.02: Select ISD-P (Issuer Security Domain)",
            emulator=emulator)
        ...
        self.send_apdu(
            init_update,
            "GSMA SGP.02: INITIALIZE UPDATE (SCP03)",
            emulator=emulator)
        ...
        self.send_apdu(
            ext_auth,
            "GSMA SGP.02: EXTERNAL AUTHENTICATE (SCP03)",
            emulator=emulator)
        ...
        store_data = [0x80, 0xE2, 0x00, 0x00,
                      len(profile_chunk)] + profile_chunk
        self.send_apdu(
            store_data,
            "GSMA SGP.02: STORE DATA (Profile Chunk)",
            emulator=emulator)
        ...
        install_apdu = [0x80, 0xE6, 0x02, 0x00,
                        len(install_params)] + install_params
        self.send_apdu(
            install_apdu,
            "GSMA SGP.02: INSTALL (Profile/Applet)",
            emulator=emulator)
        ...
        self.send_apdu(
            get_status,
            "GSMA SGP.02: GET STATUS (Verify Profile)",
            emulator=emulator)

        # Note: In production, all cryptographic values must be generated
        # securely and profile data must be constructed per GSMA SGP.02 and
        ...
        for key, fid in tmo_files.items():
            if key in self.config:
                ...
                self.send_apdu(
                    select_ef,
                    f"[T-Mobile MVNO] Select EF_{key}",
                    emulator=emulator)
                try:
                    data_bytes = [int(x, 16) for x in self.config[key].split()]
                except Exception:
                    self.log(
                        f"Invalid hex data for {key} in config. Skipping.")
                    continue
                if key == "IMSI":
                    imsi_str = ''.join(f"{b:02X}" for b in data_bytes)
                    if not imsi_str.startswith(required_mccmnc):
                        self.log(
                            f"[T-Mobile MVNO] IMSI must start with {required_mccmnc}. Provided: {imsi_str}")
                        continue
                if key == "SPN":
                    spn_ascii = self.config[key].encode('ascii')
                    data_bytes = list(
                        spn_ascii[:16]) + [0xFF] * (16 - len(spn_ascii[:16]))
                upd = [0x00, 0xD6, 0x00, 0x00, len(data_bytes)] + data_bytes
                self.send_apdu(
                    upd,
                    f"[T-Mobile MVNO] Update EF_{key} (from config)",
                    emulator=emulator)
            else:
                self.log(
                    f"[T-Mobile MVNO] {key} not set in config. Skipping EF_{key} update.")

        # --- Carrier Certificate Injection (eSIM, optional) ---
        # If CERT and PRIVKEY are present, perform a placeholder secure
        # injection step
        if "CERT" in self.config and "PRIVKEY" in self.config:
            self.log(
                "[T-Mobile MVNO] Injecting carrier certificate and private key (placeholder, automate in production)")
        ...
        if export_android:
            ...
            self.log(
                f"[Android Export] Base64 for QR: {b64[:60]}... (truncated)")
    ...
    def configure_emergency_profile_full(self, profile_name: str, region: str,
        ...
        if not self.emergency_manager:
            self.log(
                "[Emergency] Emergency manager not available. Install emergency_functions module.")
            return False
        ...
        try:
            ...
            if not success:
                self.log(
                    f"[Emergency] Failed to configure profile: {profile_name}")
                return False
            ...
            self.log(
                f"[Emergency] Profile configured successfully: {profile_name}")
            return True
...
def lint_code():
    ...
    try:
        result = subprocess.run(
            [sys.executable, '-m', 'flake8', __file__], capture_output=True, text=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="PCSC SIM/eSIM Issuer Tool - Full Standards Support")
    subparsers = parser.add_subparsers(dest="command", required=True)

    # eSIM/GSMA SGP.02/3GPP TS 31.102
    issue_parser = subparsers.add_parser(
        "esim-issue", help="Issue an eSIM using GSMA SGP.02/3GPP TS 31.102")
    ...
    issue_parser.add_argument(
        "--emulator",
        action="store_true",
        help="Run in emulator mode (no real card required)")
    issue_parser.add_argument(
        "--export-android",
        type=str,
        help="Export config for Android eSIM import (filename)")
    issue_parser.add_argument(
        "--frequency",
        type=str,
        help="Set radio frequency band/profile (e.g., 900MHz, 2100MHz, b66, n78, b20, b28, n77, n79, ALL). For emergency: b20, b28, n78, n77, etc.")
    issue_parser.add_argument(
        "--mode",
        type=str,
        help="Set operational mode (e.g., GSM, UMTS, LTE, 5G, TEST, PRODUCTION)")
    issue_parser.add_argument(
        "--signal-power",
        type=str,
        help="Set signal power in dBm (e.g., 23, 20, 10)")
    issue_parser.add_argument(
        "--emergency-profile",
        action="store_true",
        help="Enable emergency services profile (e.g., 911/112/PSAP-only)")
    issue_parser.add_argument(
        "--generate-qr",
        nargs=2,
        metavar=(
            "SMDP_ADDRESS",
            "ACTIVATION_CODE"),
        help="Generate eSIM QR code with SM-DP+ address and activation code")

    # Classic SIM/3GPP TS 51.011/ETSI TS 102 221
    sim_parser = subparsers.add_parser(
        "sim-issue", help="Issue a classic SIM using 3GPP TS 51.011/ETSI TS 102 221")
    ...
    # Legacy 3G/UMTS/USIM
    umts_parser = subparsers.add_parser(
        "umts-issue", help="Issue a USIM/3G card (legacy/3GPP TS 31.102)")
    umts_parser.add_argument(
        "--reader",
        type=int,
        default=0,
        help="PCSC reader index (default: 0)")
    ...
    config_parser = subparsers.add_parser(
        "set-config", help="Set a configuration value for issuance")
    ...
    qr_parser = subparsers.add_parser(
        "generate-qr", help="Generate eSIM QR code per GSMA SGP.22")
    qr_parser.add_argument(
        "smdp_address",
        type=str,
        help="SM-DP+ server address (e.g., sm-dp+.example.com)")
    qr_parser.add_argument(
        "activation_code",
        type=str,
        help="Activation code for the eSIM profile")
    qr_parser.add_argument(
        "--output",
        type=str,
        default="esim_qr.png",
        help="Output filename for QR code (default: esim_qr.png)")
    ...
    if args.command == "esim-issue":
        ...
        issuer.set_config("IMSI", ' '.join(
            [imsi[i:i + 2] for i in range(0, len(imsi), 2)]))
        ...
        if hasattr(args, "frequency") and args.frequency:
            ...
            print(
                f"[Automation] Frequency set to: {
                    args.frequency} (bands: {
                    args.frequency})")
        ...
        if hasattr(args, "generate_qr") and args.generate_qr:
            try:
                ...
                lpa_string = SimCardIssuer.generate_esim_qr_code(
                    smdp_address, activation_code, qr_filename)
        ...
        if args.emulator:
            issuer.issuer_mode(
                emulator=True,
                export_android=args.export_android)
            lint_code()
            return
